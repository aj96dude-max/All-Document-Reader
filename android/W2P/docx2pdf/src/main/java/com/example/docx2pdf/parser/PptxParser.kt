...
class PptxParser : DocumentParser {
    ...
    override suspend fun parse(context: Context, inputUri: Uri): UnifiedDocumentState {
        return withContext(Dispatchers.IO) {
            ...
            val zipCopy = File(context.cacheDir, "pptx_src_${System.currentTimeMillis()}.pptx")
            var slideWidthPx = 960.0
            var slideHeightPx = 540.0
            
            try {
                tempDir.mkdirs()
                extractZip(context, inputUri, tempDir, zipCopy)
                ...
                if (slideFiles.isEmpty()) {
                    throw IllegalArgumentException("Invalid PPTX: no slides found")
                }
                ...
                val pagesFlow = flow {
                    try {
                        for (slideFile in slideFiles) {
                            ...
                            var layoutShapes = emptyList<SlideShape>()
                            var layoutBgColor: String? = null
                            if (layoutFile != null && layoutFile.exists()) {
                                val result = parseShapeTree(
                                    FileInputStream(layoutFile), factory, themeColorMap, layoutRels, mediaDir,
                                    inheritedPh = masterPh, slideW = slideWidthPx, slideH = slideHeightPx,
                                    renderOnlyBackgroundShapes = true
                                )
                                layoutShapes = result.shapes
                                layoutBgColor = result.bgColor
                            }
                            ...
                            val finalBg = slideResult.bgColor ?: layoutBgColor ?: masterBgColor ?: "#FFFFFF"
    ...
    private fun extractZip(context: Context, inputUri: Uri, tempDir: File, zipCopy: File) {
        context.contentResolver.openInputStream(inputUri)?.use { stream ->
            FileOutputStream(zipCopy).use { out -> stream.copyTo(out) }
        } ?: throw IllegalArgumentException("Could not open InputStream for $inputUri")

        try {
            ZipFile(zipCopy).use { zf ->
                val canonicalDestDir = tempDir.canonicalPath
                val entries = zf.entries()
                while (entries.hasMoreElements()) {
                    val entry = entries.nextElement()
                    val destFile = File(tempDir, entry.name.replace('\\', '/'))
                    val canonicalDestFile = destFile.canonicalPath
                    if (!canonicalDestFile.startsWith(canonicalDestDir + File.separator)) continue
                    if (entry.isDirectory) {
                        destFile.mkdirs()
                    } else {
                        destFile.parentFile?.mkdirs()
                        zf.getInputStream(entry).use { input ->
                            FileOutputStream(destFile).use { fos -> input.copyTo(fos) }
                        }
                    }
                }
            }
                    ...
                    when (val tag = parser.name) {
                        ...
                        "blip" -> {
                            ...
                            for (i in 0 until parser.attributeCount) {
                                ...
                                if (attrName.equals("embed", true) || attrName.endsWith("embed", true)) {
                                    embedId = parser.getAttributeValue(i)
                                    break
                                }
                            }
                            if (embedId == null) {
                                embedId = parser.getAttributeValue(
                                    "http://schemas.openxmlformats.org/officeDocument/2006/relationships",
                                    "embed"
                                )
                            }
                            var target = embedId?.let { rels[it]?.target }
                            if (target != null && !target.exists() && mediaDir.exists()) {
                                target = File(mediaDir, target.name)
                            }
                            if (target != null && target.exists()) {
                                val ext = target.extension.lowercase()
                                if (ext in setOf("emf", "wmf", "svg")) {
                                    currentImageFile = null
                                    currentMimeType = null
                                } else {
                                    currentMimeType = if (ext in setOf("png", "gif", "bmp", "webp")) {
                                        "image/png"
                                    } else {
                                        "image/jpeg"
                                    }
